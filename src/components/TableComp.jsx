...
function SavedBtn({data}) {
  let {saveCoin,allCoins} =useContext(StorageContext)
function handleClick(e){
  e.preventDefault()
saveCoin(data.id)
}
  return (
    <button className="outline-none bottom-0 bg-none  cursor-pointer" onClick={(e)=>handleClick(e)}>
      <svg
        className={`
        ${allCoins.includes(data.id)?"fill-cyan":"fill-gray-100"}
        hover:fill-cyan ml-1.5 w-[1.5rem]`}
        width="30"
        height="30"
        viewBox="0 0 30 30"
        fill="cyan"
              ...
              {cryptoData?.map((data, id) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td
                      className={
                        Number(price_change_percentage_1h_in_currency) > 0
                    ...
                    <td
                      className={
                        Number(price_change_percentage_24h_in_currency) > 0
                          ? "text-green py-4 hidden lg:table-cell"
                          : "text-red py-4 hidden lg:table-cell"
                      }
                    >
                    ...
                    <td
                      className={
                        Number(price_change_percentage_7d_in_currency) > 0
                          ? "text-green py-4  hidden lg:table-cell"
                          : "text-red py-4 hidden lg:table-cell"
                      }
                    >
